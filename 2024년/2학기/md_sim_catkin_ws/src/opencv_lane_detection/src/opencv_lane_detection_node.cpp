...
void detectLines(Mat& img, const Mat& canny_img)
{
    vector<Vec4i> lines;
    HoughLinesP(canny_img, lines, 1, CV_PI / 180, 50, 50, 10); // Adjust parameters as needed

    // Drawing lines is removed as per your request
...
void lane_detection(Mat& img, Mat& mat_image_canny_edge, Mat& mat_image_roi)
{
    ...
    // Detect lines in the ROI image (but do not draw them on the original image)
    detectLines(mat_image_roi, mat_image_canny_edge);
